...
function estDansListeBlanche(email_recus){
  for(let i = 0; i < email_recus.length; i++){

    for(let j = 0; j < liste_blanche.length; j++){
      if(email_recus[i].from === liste_blanche[j]){
        return true
      } 
    }
    }
...
function filtrerEmailRecus(email_recus){
  for(let i = 0; i < email_recus.length; i++){
      if(estDansListeBlanche(email_recus[i].from === true)){
        liste_blanche.push(recu[i].from)
    }
    }
